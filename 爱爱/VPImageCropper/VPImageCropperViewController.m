...
//    self.ratioView.layer.cornerRadius=self.cropFrame.size.width/2;//修改圆角
...
- (void)initControlBtn {
    ...
    confirmBtn.titleLabel.textColor = [UIColor grayColor];
...
//图片重绘切圆
- (UIView*)cutViewWithOrView:(UIView *)orView WithRadius:(int)radius
{
    UIView *newView =[[UIView alloc]initWithFrame:orView.frame];
    
    UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:newView.frame byRoundingCorners:UIRectCornerBottomLeft | UIRectCornerBottomRight cornerRadii:CGSizeMake(radius, radius)];
    CAShapeLayer *maskLayer = [[CAShapeLayer alloc] init];
    maskLayer.frame = newView.frame;
