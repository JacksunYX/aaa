...
- (void)setModel:(CalendarDayModel *)model
{
    ...
    if (model.style==CellSelectedNotClick) {
        
        imgview.image=[UIImage imageNamed:@"haveSelected_icon"];
        
    }else{
    
        imgview.image = [UIImage imageNamed:@"getIn_icon"];
    
    }

    imgview.center=day_lab.center;
...
//            day_lab.textColor = [UIColor whiteColor];
        ...
        case CellDayTypeNotClick:   //未被点击状态
            [self hidden_NO];
            if (model.holiday) {
                day_lab.text = model.holiday;
                day_lab.textColor = [UIColor orangeColor];
        ...
        case CellSelectedNotClick:    //已经被选定，且无法再被选定
            [self hidden_NO];
            day_lab.hidden=YES; //隐藏时间
//            day_lab.text = [NSString stringWithFormat:@"%lu",(unsigned long)model.day];
//            day_lab.textColor = [UIColor whiteColor];
            day_title.text = model.Chinese_calendar;
            imgview.hidden = NO;
