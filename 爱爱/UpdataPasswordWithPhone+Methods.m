//
//  UpdataPasswordWithPhone+Methods.m
//  爱爱
//
...
#import "UpdataPasswordWithPhone+Methods.h"
...
#import "UserDataViewController.h"  //用户信息页
#import "UIViewController+JTNavigationExtension.h"

@implementation UpdataPasswordWithPhone (Methods)
...
-(void)creatBaseControll        //加载控件
{
    ...
    while ([userResult next]) { //将数据获取并保存到模型中
        ...
        phoneNum = [userResult stringForColumn:@"phoneNum"];
        
    }
    
    //"手机号"标签
    originalPhoneNum =[[UILabel alloc]initWithFrame:CGRectMake(20,NavigationBarHeight+20, 100, 40)];
    originalPhoneNum.font = [UIFont fontWithName:PingFangSCX size:18];
    originalPhoneNum.text=@"手机号:";
    originalPhoneNum.dk_textColorPicker=DKColorWithColors([UIColor blackColor], ContentTextColorNight);
    ...
    phone.dk_textColorPicker=DKColorWithColors([UIColor blackColor], ContentTextColorNight);
    phone.font = [UIFont fontWithName:PingFangSCX size:18];
    phone.numberOfLines=1;
    phone.textAlignment=1;
    if (phoneNum.length>0) {
        
        [phone setText:[phoneNum stringByReplacingCharactersInRange:NSMakeRange(3, 4) withString:@"xxxx"]];
        
    }
    [phone sizeToFit];
    phone.frame=CGRectMake(Width-20-phone.frame.size.width, originalPhoneNum.frame.origin.y+originalPhoneNum.frame.size.height/2-phone.frame.size.height/2, phone.frame.size.width, phone.frame.size.height);
    ...
    getTest.titleLabel.font=[UIFont fontWithName:PingFangSCX size:16];//修改字体大小
    ...
    getTest.layer.borderColor = [UIColor lightGrayColor].CGColor;
    getTest.layer.borderWidth = 0.3;
    getTest.layer.masksToBounds = YES;
...
//    getTest.layer.shadowOpacity=0.3;
    ...
    //确认按钮
    updataBtn =[[UIButton alloc]initWithFrame:CGRectMake(20, testCode.frame.origin.y+testCode.frame.size.height+40, Width-20*2, 40)];
    updataBtn.titleLabel.font=[UIFont fontWithName:PingFangSCX size:18];
...
//    updataBtn.layer.shadowOpacity=0.5;
    ...
    self.viewArr =[NSArray arrayWithObjects:newPassword,repeatPassword,testCode, nil];
...
-(void)delayTimeToExecute   //延时执行
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        NSArray *controllersArr =self.jt_navigationController.jt_viewControllers;
        
        for (UIViewController *cv in controllersArr) {
            
            if ([cv isKindOfClass:[UserDataViewController class]]) {
...
-(IBAction)getTestCode:(UIButton *)sender      //获取手机验证码点击事件
{
    ...
    [self sendRequestToGetTestCodeWithPhoneNumber:phoneNum];
